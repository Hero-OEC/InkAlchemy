...
- **WordProcessor Reset Bug Fix**: Fixed critical issue in race create/edit forms where WordProcessor would reset to empty content while typing. Root cause was onChange handlers setting description to null on JSON parse errors during debounced saving. Fixed by preserving previous valid state instead of resetting, eliminating "Block paragraph skipped because saved data is invalid" errors and content loss during typing
- **Complete Width Standardization**: Successfully standardized all page layouts from inconsistent max-w-4xl and max-w-6xl to unified max-w-7xl across entire application. Updated all skeleton loading components to match their respective page widths, ensuring perfect alignment during loading states. Fixed timeline skeleton component and all form/details pages for consistent user experience
- **ContentCard Text Extraction Fix**: Fixed ContentCard component displaying raw Editor.js JSON data by creating utility function to extract only plain text from paragraph blocks. Ignores all formatting elements (headings, lists, quotes, images, tables) and strips HTML tags, showing clean readable descriptions across all entity cards
- **Custom Favicon Implementation**: Implemented user-uploaded official InkAlchemy SVG favicon with professional design, proper proportions, and brand-consistent golden potion bottle logo with lightning bolt pattern